 ...
 */
public class getRevenueByTimeResponseHandler implements RequestHandler{
    @Override
    public boolean canHandle(HandlerInput handlerInput) {
        IntentRequest request = (IntentRequest) handlerInput.getRequestEnvelope().getRequest();
        Intent intent = request.getIntent();
        if(intent.getName().matches(request_RevenueByTime))
        {
            return true;
        }
        return false;
        ...
        else
        {
            try {
                ...
                if(Utils.retrieveServletErrors(handlerInput, jsonOutputString) == null)
                {
                    ...
                    return tellWithCard(handlerInput, "CIP Revenue","Total Revenue for all insurers was " + jsonOutput.getString("outputRevenue") + "€ between " + formatDateToAmerican(startDate) + " and " + formatDateToAmerican(endDate));
                }
                else
                {
