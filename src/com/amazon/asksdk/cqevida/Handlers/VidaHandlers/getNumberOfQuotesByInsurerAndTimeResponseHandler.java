...
public class getNumberOfQuotesByInsurerAndTimeResponseHandler implements RequestHandler {

    @Override
    public boolean canHandle(HandlerInput handlerInput) {
        IntentRequest request = (IntentRequest) handlerInput.getRequestEnvelope().getRequest();
        Intent intent = request.getIntent();
        if(intent.getName().matches(request_NumberOfQuotesByInsurerAndTime))
        {
            return true;
        }
        return false;
        ...
        else
        {
            try {
                ...
                if(Utils.retrieveServletErrors(handlerInput, jsonOutputString) == null) {
                    ...
                    return tellWithCard(handlerInput, "CIP Quotes", "There have been " + jsonOutput.getString("outputQuoteCount") + " quotes from " + insurer + " between " + formatDateToAmerican(startDate) + " and " + formatDateToAmerican(endDate));
                }
                else
                {
